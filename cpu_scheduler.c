...
PCB create_process(int pid, const char *name, EmergencyType type,
...
void print_pcb(PCB *p) {
    printf("  PID %-3d | %-28s | %-9s | Priority %-2d | "
           ...
           p->pid, p->name, emergency_to_str(p->emergency_type),
           p->priority, p->arrival_time, p->burst_time,
           state_to_str(p->state));
}
...
PCB *peek(ReadyQueue *q) {
    return q->front ? q->front->process : NULL;
}
...
SchedulingResult run_round_robin(PCB processes[], int n, int quantum,
    ...
    for (int i = 0; i < n; i++) total_burst += work[i].burst_time;

    char log_buf[128];
    snprintf(log_buf, sizeof(log_buf),
             "Round Robin started. n=%d, quantum=%d", n, quantum);
    log_event(log_buf);
    ...
    while (done < n) {
        if (is_empty(queue)) {
            ...
            for (int i = 0; i < n; i++) {
                if (work[i].state != TERMINATED &&
                    work[i].arrival_time > time)
                    next_arrival = (work[i].arrival_time < next_arrival)
                                   ? work[i].arrival_time : next_arrival;
            }
        ...
        int slice = (curr->remaining_time < quantum)
        ...
        if (curr->remaining_time == 0) {
            ...
            snprintf(log_buf, sizeof(log_buf),
                     "PID %d (%s) TERMINATED at t=%d | WT=%d TAT=%d",
                     curr->pid, curr->name, time,
                     curr->waiting_time, curr->turnaround_time);
            log_event(log_buf);
        } else {
...
SchedulingResult run_priority_scheduling(PCB processes[], int n,
    ...
    while (done < n) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (work[i].arrival_time > time) continue;
        ...
        snprintf(log_buf, sizeof(log_buf),
                 ...
                 curr->pid, curr->name, time, curr->priority,
                 curr->waiting_time, curr->turnaround_time);
        log_event(log_buf);
    }
...
void display_gantt_chart(GanttEntry gantt[], int len) {
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (gantt[i].pid == -1) {
            printf(" IDLE");
            for (int w = 0; w < width * 2 - 4; w++) printf(" ");
        } else {
            printf(" P%-2d", gantt[i].pid);
            for (int w = 0; w < width * 2 - 3; w++) printf(" ");
        }
        printf("|");
    }
    ...
    if (len > 0) printf("%d", gantt[len - 1].end);
    printf("\n\n");
}
...
void display_metrics_table(PCB processes[], int n) {
    ...
    printf("  ║ AT=Arrival Time  BT=Burst Time  WT=Waiting  TAT=Turnaround  RT=Response ║\n");
    printf("  ╚═══════════════════════════════════════════════════════════════════════════╝\n");
}
...
void cpu_scheduling_module(void) {
    ...
    if (choice == 1) {
        ...
        for (int i = 0; i < n; i++) print_pcb(&original[i]);
    }
    ...
    while (running) {
        ...
        case 1: {
            ...
            SchedulingResult rr = run_round_robin(processes, n, quantum,
                                                  gantt, &gantt_len);
        ...
        case 2: {
            ...
            SchedulingResult ps = run_priority_scheduling(processes, n,
                                                          gantt, &gantt_len);
            display_gantt_chart(gantt, gantt_len);
            display_metrics_table(processes, n);
        ...
        case 3: {
            ...
            printf("\n  ══════════ ROUND ROBIN ══════════\n");
            SchedulingResult rr = run_round_robin(proc_rr, n, quantum,
                                                  g_rr, &glen_rr);
            ...
            printf("\n  ══════════ PRIORITY SCHEDULING ══════════\n");
            SchedulingResult ps = run_priority_scheduling(proc_ps, n,
                                                          g_ps, &glen_ps);
            ...
            printf("        for equal-priority tasks). Priority Scheduling minimises\n");
            printf("        turnaround for the highest-urgency emergencies (Ambulance\n");
            printf("        > Fire > Police) but may starve low-priority processes.\n\n");
            break;
        }
        ...
        case 5:
            ...
            scanf("%d", &choice);
            if (choice == 1) n = get_processes_from_user(original);
            else             n = load_demo_scenario(original);
            break;
